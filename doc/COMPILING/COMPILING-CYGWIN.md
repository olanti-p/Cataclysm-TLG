...
This guide contains instructions for compiling Cataclysm-DDA on Windows under Cygwin. **PLEASE NOTE:** These instructions *are not intended* to produce a redistributable copy of CDDA. Please download the official builds from the website or [cross-compile from Linux](https://github.com/Cataclysm-TLG/Cataclysm-TLG/blob/master/doc/COMPILING/COMPILING.md#cross-compile-to-windows-from-linux) if that is your intention.
...
1. Clone the Cataclysm-TLG repository with following command:

**Note:** This will download the entire CTLG repository and all of its history (3GB). If you're just testing, you should probably add `--depth=1` (~350MB).
...
```bash
cd /cygdrive/c/dev/Cataclysm-TLG
./cataclysm-tiles
```

There is no functionality for running Cygwin-compiled CTLG from outside of UXTerm.
